...
class OVLLMBeamSearch(llm.OVLLM):
    def _patch_model(self, model):
        ...
        h_topk = opset.topk(h_lgsoftmax, topk_K, axis=np.int32(-1), mode="max", sort="value")
        next_score = opset.result(h_topk.output(0), name='next_score')
        next_indicies = opset.result(h_topk.output(1), name='next_indicies')

        model.add_parameters([oshape, beam_scores, topk_K])
    ...
    def _generate(self,
                  model,    # ompiled_model,
                  kv_cache, # [2 * n_layers, max_kv_len, batch_size, n_head, head_size]
                  eos_token_id,
                  pad_token_id,
                  input_ids,
                  attention_mask,
